...
# You should see something like "ch?[1:7]" in the 'Environment'
...
# equal sign '=' in other programming lang?ages. The c() is a
...
# same ty?e: character (most programmers call these strings),
...
# Each number corresponds to the in?ex of the entry in the vector.
...
# ANOTHER HELP?UL TIP: You can add values to a vector.
...
# 'udacious' and ?un the code. Be sure to use quotes around it.
...
# Notice how R updates 'udacio?s' in the workspace.
...
# You just created a new vector called mystery. What do ?ou
...
# Here we get a logi?al (or boolean) vector that tells us
...
# runn?ng the line of code below?
...
# It's ?our Udacious Instructors for the course!
...
# 6. Alright, all mystery aside...let's dive in?o some data!
...
# You should see mtcars app?ar in the 'Environment' tab with
...
# called a ?ata frame. Run the next command to see what variables
# are in the data set and to fully load the data set as an
# object in R. You should see <Promise> disappear when you
# run the next line of code.

# Visit http://cran.r-project.org/doc/manuals/r-releas?/R-lang.html#Promise-objects
...
# names in the data set. You m?ght notice that the car names
...
# the data frame and the number of variables on each observ?tion.
...
# and see if you can figure out wh?t they do.
...
# (all of these variables are numerics) and so?e values for each
# variable.
...
# of many oth?r programming languages.)
...
#      c, nchar, data, str, dim, nam?s, row.names, head, and tail.
...
# mile? per gallon. This is the mpg column in the data frame.
